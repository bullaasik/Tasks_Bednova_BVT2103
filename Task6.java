...
public class Task6 {
    ...
    public static boolean isVowel(Character a) { //проверка буквы на гласную
        ...
        if ("a".equals(strA) || "i".equals(strA) || "o".equals(strA) || "e".equals(strA) || "u".equals(strA) || "y".equals(strA))
            return true;
        else return false;
    }
    ...
    public static boolean validColor(String s){ //верный формат параметров RGB(A)
        String[] opt = (s.substring(s.indexOf("(") + 1, s.indexOf(")"))).split(","); //substring возвращает подстроку строки между двумя индексами или от одного индекса до конца строки
        int optInt = 0;
        double optDoub = 0;
        if (s.contains("rgba") && opt.length != 4) //проверка - содержит ли указанную последовательность символов
            return false;
    ...
    public static String stripUrlParams(String url, String[] paramsToStrip) { //удаление повторяющихся параметров URL
        if (!url.contains("?"))
            ...
            noRepeat.addAll(Arrays.asList(paramsToStrip)); //adAll - добавить элементы одной коллекции в другую
            int k = 1;
